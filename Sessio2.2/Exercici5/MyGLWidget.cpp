...
void MyGLWidget::modelTransform () 
{
  // Matriu de transformació de model
  glm::mat4 transform (1.0f);
  transform = glm::translate(transform, glm::vec3(9.5,-3.5,0));
...
void MyGLWidget::viewTransform() {
    glm::mat4 View = glm::lookAt(OBS, centrat, UP);
...
void MyGLWidget::resizeGL (int w, int h) 
{
  ...
  ra = ra_v;
  /*if (ra_v < 1) {
    FOV = 2*glm::atan(glm::tan((FOV)/ra_v));
    }*/
  if(not ortogonal) {
      if(ra < 1) FOV = 2*glm::atan(glm::tan(glm::asin(radi/diametre))/ra);
      else FOV = M_PI/2.0;
  } else {
      //float a_w = ra_v * float(h) * 2 * R;
      //inc_a = a_w - float(w);  
  }
}
...
void MyGLWidget::ini_camera_ortogonal() {
    ...
    //alfa = glm::asin(radi/diametre);
    //std::cout << alfa << std::endl;
    OBS = centrat+((diametre)*glm::vec3(0,0.5,1));
